...
def deleteLineFeed(str, lineNumList):
    blankList = []
    ...
    ele = 0
    while ele < len(str):
        if str[ele] == '' or str[ele] == '\t' or str[ele] == ' ':
            str.pop(ele)
            ele -= 1
        ele += 1
    ...
    for i in range(len(str)):
        ...
        while i + plus in blankList:
            plus += 1
        retList.append(lineNumList[i + plus])
...
class Filter:
    ...
    def deleteComment(self, token, lineNumList):
        lineList = []
        lineNum = 0
        ...
        while 1:
            ...
            for i in range(self.file[startIdx:(startIdx + endIdx)].count('\n')):
                lineList.append(lineNumList[lineCount + lineNum + i + 1])
    ...
    def tokenizing(self, lineNumList):
        stopWords = ['', ' ', '{', '}']
        retList = []
        blankList = []
        lineList = []
        ...
        for i in range(len(retList)):
            ...
            while i + plus in blankList:
                plus += 1
            lineList.append(lineNumList[i + plus])
